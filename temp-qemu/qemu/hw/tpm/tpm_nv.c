 ...
 * -------------------------------------------------------------------*/
static inline bool
impl_supports_counter (TPM2State *s)
{
  (void)s;
  return true;
}
static inline bool
impl_supports_setbits (TPM2State *s)
{
  (void)s;
  return true;
}
static inline bool
impl_supports_undefine_policy_delete (TPM2State *s)
{
  (void)s;
  return true;
}
...
/* ---------------------------------------------------------------------
 * TPM2_NV_DefineSpace Response Struct
 * -------------------------------------------------------------------*/
typedef struct
{
 ...
 * -------------------------------------------------------------------*/
static inline UINT8
nv_index_type (TPMA_NV attrs)
{
  return (UINT8)(attrs.nvType);
}

static inline bool
any_read_bit (TPMA_NV a)
{
  return (a.ppRead || a.ownerRead || a.authRead || a.policyRead);
}

static inline bool
any_write_bit (TPMA_NV a)
{
  return (a.ppWrite || a.ownerWrite || a.authWrite || a.policyWrite);
}

/* ---------------------------------------------------------------------
 * TPM2_NV_DefineSpace implementation
 * -------------------------------------------------------------------*/
/*
 *  @brief Verifies all parameters are valid, allocates a new NV index, and
 *  reigsters it into TPM state.
 *  @param s Pointer to global TPM State
 *  @param authHandle Handle authorizing the command (influences which kinds of attributes are allowed)
 *  @param auth Authorization data for the new NV index
 *  @param public Public area describing the NV index containing its metadata (size, handle, attributes)
 *  @return TPM_RC_SUCCESS on success, error code on failure.
 * */
TPM_RC
tpm2_nv_define_space (TPM2State             *s,
