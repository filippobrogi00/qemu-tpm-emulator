...
static void
tpm2_generate_random (TPM2State *s)
{
  RAND_bytes (s->random_data, sizeof (s->random_data));
}

static void
tpm2_generate_rsa_key (TPM2State *s)
{
  if (s->rsa_key)
    {
      RSA_free (s->rsa_key);
...
static uint64_t
tpm2_mmio_read (void *opaque, hwaddr addr, unsigned size)
{
  TPM2State *s = opaque;
  switch (addr)
    {
...
static void
tpm2_mmio_write (void *opaque, hwaddr addr, uint64_t value, unsigned size)
{
  TPM2State *s = opaque;
  switch (addr)
    {
...
static void
tpm2_class_init (ObjectClass *klass, void *data)
{
...
static void
tpm2_register_types (void)
{
  fprintf (stderr, "[DEBUG] tpm2_register_types called\n");
  // sysbus_register_type(&tpm2_info);
  (void)type_register_static (&tpm2_info);
  fprintf (stdout, "[DEBUG] after type_register_static call\n");
}
